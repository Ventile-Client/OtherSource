...
namespace VentileClient
{
    public partial class MainWindow : Form
    {
        ...
        // 7 = Custom DLL
        // 8 = Default DLL
        // 9 = Custom Resource loc?
        // 10 = Custom Resource Location
        // 11 = Custom Background Image?
        // 12 = Custom Background Image Location
        // 13 = Toasts
        // 14 = Rounded Buttons
        // 15 = Theme
        ...
        public MainWindow()
        {
            ...
            Updater updateCheck = new Updater();
            updateCheck.Show();
        ...
        string[,] packInfo =
        {
            { "3256f0cd-498a-4f97-a7ca-89dbeeeee4b8", " 1, 0, 0 " }, // Black Cape
            { "51fabcfb-0636-4aa6-a935-2c34add462e4", " 1, 0, 0 " }, // White Cape
            { "f495127b-83db-4e35-b93e-6c7f4b046a0f", " 1, 0, 0 " }, // Pink Cape
            { "de9c860b-3ca6-4f48-8d81-1c8f4c65b8e9", " 1, 0, 0 " }, // Blue Cape
            { "fc6bcc91-ee3c-4b97-bfcf-2243de323870", " 1, 0, 0 " }, // Yellow Cape
            { "2fc8d46c-3652-4836-adfc-c1d2a43d5777", " 1, 0, 0 " }, // Rick Cape

            { "982e6a3a-56f2-4e78-b0d5-b5eb9df0792b", " 1, 0, 0 " }, // Black Mask
            { "03910601-ed59-4fb6-add5-f3b8cf27a8ff", " 1, 0, 0 " }, // White Mask
            { "f4b600af-c2b3-43aa-b775-dec476f56829", " 1, 0, 0 " }, // Pink Mask
            { "0af582ae-018f-47c4-92a9-db0643279fd0", " 1, 0, 0 " }, // Blue Mask
            { "9891cbf5-d3d4-4d2d-94d9-cfeea717c2f9", " 1, 0, 0 " }, // Yellow Mask
            { "6f7a314a-ea8c-4da0-ba38-4af023a0762e", " 1, 0, 0 " }, // Rick Mask

            { "ed7b6991-50af-42c7-b302-84543a97e72b", " 1, 0, 0 " }, // Glowing Cape
            { "57c6383e-7cb8-40b8-88af-8cbc18707c63", " 1, 0, 0 " }, // Sliding Cape

            { "a6953918-a86c-43bb-a3af-ae6377bdba63", " 1, 0, 0 " }, // Wavy Capes
            { "64564ae0-011b-4c2f-a87d-449c8119f234", " 0, 0, 1 " } // Animated Kagune
        };

        private bool cosmeticsEnabled()
        {
            if (!File.Exists(configCS.ResourcePackLoc + @"\..\minecraftpe\global_resource_packs.json")) return false;

            string[] noGlobalTemp = File.ReadAllLines(configCS.ResourcePackLoc + @"\..\minecraftpe\global_resource_packs.json");

            char quote = '"';

            bool chk = false;

            for (int i = 0; i < noGlobalTemp.Length; i++)
            {
                for (int j = 0; j < packInfo.Length / 2; j++)
                {
                    if (noGlobalTemp[i].Trim() == string.Format("{0}pack_id{0} : {0}{1}{0},", quote, packInfo[j, 0]))
                        chk = true;
                }
            }
        ...
        private List<string> readPack()
        {
            if (!File.Exists(configCS.ResourcePackLoc + @"\..\minecraftpe\global_resource_packs.json")) return new List<string> { "ERRFAILERR" };

            string[] temp = File.ReadAllLines(configCS.ResourcePackLoc + @"\..\minecraftpe\global_resource_packs.json");

            List<string> returner = new List<string>(temp);
        ...
        private bool cosmeticEnabledByString(string id)
        {
            if (!File.Exists(configCS.ResourcePackLoc + @"\..\minecraftpe\global_resource_packs.json")) return false;
            ...
            for (int i = 0; i < noGlobalTemp.Length; i++)
            {
                if (noGlobalTemp[i].Trim() == string.Format("{0}pack_id{0} : {0}{1}{0},", quote, id))
                    chk = true;
            }
        ...
        private void removeCosmetics(string id)
        {
            ...
            if (cosmeticEnabledByString(id))
            {
                string[] noGlobalTemp = readPack().ToArray();

                if (noGlobalTemp[0] == "ERRFAILERR") return;

                List<string> rpList = new List<string>(noGlobalTemp);
                ...
                File.WriteAllLines(configCS.ResourcePackLoc + @"\..\minecraftpe\global_resource_packs.json", rpList.ToArray());
            }
        }

        string versionForCosmeticAdd;
        ...
        private void addCosmetic(string id)
        {
            ...
            for (int i = 0; i < packInfo.Length / 2; i++)
            {
                if (packInfo[i, 0] == id)
                {
                    versionForCosmeticAdd = packInfo[i, 1];
                }

            }

            rpList.Insert(1, "},");
            rpList.Insert(1, string.Format("{0}version{0} : [{1}]", quote, versionForCosmeticAdd));
            rpList.Insert(1, string.Format("{0}pack_id{0} : {0}{1}{0},", quote, id));
            rpList.Insert(1, "{");
        ...
        private void resetCapes()
        {
            ...
            if (internet)
            {
                removeCosmetics(packInfo[0, 0]);
                removeCosmetics(packInfo[1, 0]);
                removeCosmetics(packInfo[2, 0]);
                removeCosmetics(packInfo[3, 0]);
                removeCosmetics(packInfo[4, 0]);
                removeCosmetics(packInfo[5, 0]);
        ...
        private void resetMasks()
        {
            ...
            if (internet)
            {
                ...
                removeCosmetics(packInfo[6, 0]);

                removeCosmetics(packInfo[7, 0]);

                removeCosmetics(packInfo[8, 0]);

                removeCosmetics(packInfo[9, 0]);

                removeCosmetics(packInfo[10, 0]);

                removeCosmetics(packInfo[11, 0]);
        ...
        private void resetExtras()
        {
            ...
            removeCosmetics(packInfo[12, 0]);
            removeCosmetics(packInfo[13, 0]);
        ...
        private void resetOthers()
        {
            ...
            removeCosmetics(packInfo[14, 0]);
            removeCosmetics(packInfo[15, 0]);
            ...
            if (importConfig.ShowDialog() == DialogResult.OK)
            {
                if (importTheme.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        conf = JsonSerializer.Deserialize<ConfigTemplate>(temp);

                        temp = File.ReadAllText(themePath);
                        them = JsonSerializer.Deserialize<ThemeTemplate>(temp);
                        ...
                        json = JsonSerializer.Serialize(them, new JsonSerializerOptions { WriteIndented = true });
